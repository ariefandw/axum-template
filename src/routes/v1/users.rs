...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "User profile updated", body = ApiResponse<UserResponse>),
...
pub async fn list_users(
    ...
    let users = sqlx::query_as::<_, User>(
        "SELECT id, name, email, email_verified, image, role, banned, created_at, updated_at FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2",
    )
    .bind(limit)
    .bind(offset)
    .fetch_all(&state.db)
